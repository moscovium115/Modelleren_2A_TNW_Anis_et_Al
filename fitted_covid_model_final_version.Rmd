...
list of changes of the parameters at certain time points to fit it to the actual data and incorporate the measurements that were taken and the rate of vaccination for the normal vaccination strategy 
```{r}
...
list of changes with no vaccination
```{r}
list.of.updaters2 <- list(
  list(
    at = 1,  
    param = list(  
      inf.prob = 0.11, 
...
list of changes with vaccination 50 days later 
```{r}
list.of.updaters3 <- list(
  list(
    at = 1,  
    param = list(  
      inf.prob = 0.11, 
...
list of changes with vaccination 55 days earlier
```{r}
list.of.updaters4 <- list(
  list(
    at = 1,  
    param = list(  
      inf.prob = 0.11, 
...
list of changes with faster vaccination 2x the rates
```{r}
list.of.updaters5 <- list(
  list(
    at = 1,  
    param = list(  
      inf.prob = 0.11, 
...
control <- control.net(type = NULL, nsteps = 280, nsims = 20,ncores = 10, infection.FUN = infection.net, progress.FUN = progress,resimulate.network = FALSE, verbose = TRUE)
#setting the initial parameters and adding a list of changes at certain times
#changing the .param.updater.list from 1 to 5 gives the different vaccination strategies
...
ggplot() +   geom_line(data = df, mapping = aes(time, pplnode*i.num, group = sim), alpha = 0.25,
  ...
  geom_line(data = df.mean, mapping = aes(time, pplnode*i.num,color = "simulation"))+ geom_line(data = predata, mapping = aes(predata$X, predata$infected_preproc, color = "data")) + xlab("time [days]") + ylab("number of infected")+ xlim(0,280)  +   scale_color_manual(name = " ", breaks = c("simulation","data"),values = c("black", "cyan"), guide = "legend") +
  theme_minimal()
ggsave(file = "infect number fast vaccine.pdf", dpi = 600)
...
ggplot() +
  ...
  geom_bands(data = df, mapping = aes(time, v.num/nodes),
             lower = 0.1, upper = 0.9, fill = "firebrick") +
  geom_line(data = df.mean, mapping = aes(time, v.num/nodes)) + xlim(c(80,280)) +
  theme_minimal()
ggsave(file = "vaccine number fast.pdf", dpi = 600)
```
